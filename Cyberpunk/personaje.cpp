...
void Personaje::actualizarNivel2(float dt)
{
    ...
    // qDebug() << "rotation=" << itemGrafico->rotation()
    // << "transformOrigin=" << itemGrafico->transformOriginPoint()
    ...
    if (estadoAnim != EstadoAnim::VIENTO_CAIDA && itemGrafico)
        itemGrafico->setRotation(0.f);
                              ...
                              bool loop, float multVelocidad)
{
    ...
    if(isSigiloActivo()){
        ...
        // 1. Hacemos que todo el dibujo sea 50% transparente (efecto fantasma)
        p.setOpacity(0.5);
        p.drawPixmap(0, 0, frame);

        // 2. Le aplicamos un tinte gris oscuro por encima
        p.setCompositionMode(QPainter::CompositionMode_SourceAtop);
        p.fillRect(frame.rect(), QColor(80, 80, 80, 100)); // R, G, B, Alfa (Gris)
...
float Personaje::getProgresoCooldownBoost()
{
    ...
    // if (cooldownBoost <= 0.f) return 1.f;

    // // Calculamos cuánto tiempo ha transcurrido desde que se usó
    // float tiempoTranscurrido = COOLDOWN_BOOST_MAX - cooldownBoost;
...
void Personaje::resetearPosicion(float rx, float ry)
{
    ...
    miraDerecha=true;       //linea agregada para corregir error, frames nivel 2
